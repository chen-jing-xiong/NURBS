...
class NURBSMultiPatch{
private:
    NURBSCurve2D** patches_; // 指针数组，存储每一片的地址
    int numPatches_;           // 当前已添加的片数
    int totalDofs_;            // 总的控制点数，首尾相邻
public:
    NURBSMultiPatch(NURBSCurve2D** patches, int n) {
        numPatches_ = n;
        totalDofs_ = 0;
        patches_ = new NURBSCurve2D*[numPatches_];    // 分配指针数组空间
    ...
    NURBSCurve2D* getPatch(int index) const {return patches_[index];}
};